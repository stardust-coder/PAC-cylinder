...
def polar_density(data, params):
    ...
    mu, kappa, alpha, beta, gamma_ = params
    c = 1 - np.tanh(kappa) * np.cos(theta - mu)
    return (x ** (alpha - 1)) * np.exp(- ((beta * x) ** gamma_) * c)

def log_normalizing_constant(mu, kappa, alpha, beta, gamma):
    def integrand(theta):
        denom = 1 - np.tanh(kappa) * np.cos(theta - mu)
        return denom ** (-alpha/gamma)

    integral, _ = quad(integrand, 0, 2 * np.pi, limit=100)
    if integral < 1e-12:
        return np.inf
    log_z = loggamma(alpha/gamma) - alpha * np.log(beta) - np.log(gamma) + np.log(integral)
    return log_z
...
def Q_function(params, data, gamma_k):
    try:
        ...
        log_unnorm = (alpha - 1) * np.log(x) - ((beta * x)**gamma_) * c
        log_f = log_unnorm - log_normalizing_constant(mu, kappa, alpha, beta, gamma_)
        return np.sum(gamma_k * log_f)
    
    except Exception as e:
        print("Exception in Q_function:", e)
        return np.inf
...
class EMAlgorithm:
    ...
    def density(self, data, params):
        ...
        log_z = log_normalizing_constant(mu, kappa, alpha, beta, gamma)
    ...
    def m_step(self, data, gamma):
        raw_weights = np.sum(gamma, axis=0)
        epsilon = 1e-6
        self.weights = np.clip(raw_weights / len(data), epsilon, 1.0)
        ...
        for k in range(self.n_components):
            ...
            mu0, kappa0, alpha0, beta0, gamma0 = self.params[k]
            
            res = minimize(
                neg_Q,
                x0=np.array([mu0, kappa0, alpha0, beta0, gamma0]),
                bounds=[(0, 2 * np.pi), (1e-3, 10), (1e-3, 10), (1e-3, 10), (1e-3, 10)],
                method='L-BFGS-B'
                # method='Powell'
            )
            if res.success:
                ...
                print(f"Warning: optimization failed for component {k}. msg: {res.message}.")
    ...
    def fit(self, data, tol=1e-4, max_iter=10000):
        ...
        for step in range(max_iter):
            ...
            for i in range(len(tmp)):
                print(self.weights[i],[x.item() if type(x) is not float else x for x in tmp[i]])
